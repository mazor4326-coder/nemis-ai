    ...
    if not is_registered:
        ...
        if not u.get('lang') or step == 'lang':
            ...
            if txt in [t['agree_btn'], "🟢 Да, я согласен / Ha, roziman"] or "roziman" in txt.lower() or "согласен" in txt.lower() or "agree" in txt.lower():
                ...
                return
            elif txt in [t['disagree_btn'], "❌ Не согласен / Rozimasman"] or "rozimasman" in txt.lower() or "не согласен" in txt.lower() or "disagree" in txt.lower() or "нет" in txt.lower():
                ban_msg = {
                    'ru': "Мы понимаем, вам просто лень. Но помните, все эти правила исключительно для вашей пользы! Выучить немецкий язык без железной дисциплины невозможно. Когда найдете в себе силы победить лень и действительно захотите добиться результата — возвращайтесь и примите правила!",
                    'uz': "Biz tushunamiz, sizga shunchaki dangasalik qilyapti. Lekin unutmang, bu qoidalarning barchasi faqat va faqat sizning foydangiz uchun! Nemis tilini temir intizomsiz o‘rganib bo‘lmaydi. Qachonki o‘zingizda kuch topib, dangasalikni yengib, natijaga erishishni chin dildan xohlasangiz — qaytib keling va qoidalarni qabul qiling!",
                    'en': "We understand, you are just being lazy. But remember, all these rules are exclusively for your benefit! It is impossible to learn German without iron discipline. When you find the strength to overcome laziness and truly want to achieve results — come back and accept the rules!",
                    'de': "Wir verstehen, Sie sind einfach faul. Aber denken Sie daran, all diese Regeln dienen ausschließlich Ihrem Vorteil! Es ist unmöglich, Deutsch ohne eiserne Disziplin zu lernen. Wenn Sie die Kraft finden, die Faulheit zu überwinden und wirklich Ergebnisse erzielen wollen — kommen Sie zurück und akzeptieren Sie die Regeln!"
                }
